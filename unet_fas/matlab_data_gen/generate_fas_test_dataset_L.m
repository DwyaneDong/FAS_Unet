function generate_fas_test_dataset_L(num_samples_per_L, L_values, snr_value, output_filename)
    ...
    %
    % 输出:
    %   5维张量: (num_L, num_samples_per_snr, M, K, 2)
    %   其中第一维是SNR值，方便按SNR测试
    ...
    if nargin < 2
        L_values = 20:20:160; % 默认SNR范围: -10dB to 20dB, 步长5dB
    end
    if nargin < 1
        num_samples_per_L = 150; % 每个SNR值100个样本
    end

    num_L = length(L_values);
    total_samples = num_L * num_samples_per_L;
    
    fprintf('开始生成测试数据集:\n');
    fprintf('  L值: [%s] 条， SNR: [%s] dB \n', num2str(L_values), num2str(snr_value));
    fprintf('  每个L样本数: %d\n', num_samples_per_L);
    fprintf('  总样本数: %d\n', total_samples);
    fprintf('  保存到: %s\n', output_filename);
    ...
    % 初始化5维张量: (num_L, num_samples_per_snr, M, K, 2)
    % all_masked_tensor = zeros(num_L, num_samples_per_L, M_fixed, K_fixed, 2);
    % all_full_tensor = zeros(num_L, num_samples_per_L, M_fixed, K_fixed, 2);

    all_masked_tensor = zeros(num_samples_per_L, M_fixed, K_fixed, 2);
    all_full_tensor = zeros(num_samples_per_L, M_fixed, K_fixed, 2);
    
    % 为兼容性也保存cell array格式
    % all_masked_channels = cell(num_L, num_samples_per_L);
    % all_full_channels = cell(num_L, num_samples_per_L);
    all_masked_channels = cell(num_samples_per_L);
    all_full_channels = cell(num_samples_per_L);

    for L_idx = 1:num_L
        current_snr = snr_value;
        fprintf('\n处理L = %d dB (%d/%d)\n', L_values(L_idx), L_idx, num_L);
        
        for sample_idx = 1:num_samples_per_L
            if mod(sample_idx, 10) == 0
                fprintf('  样本 %d/%d\n', sample_idx, num_samples_per_L);
            end
            ...
            % 存储到张量
            % all_masked_tensor(L_idx, sample_idx, :, :, :) = input_data;
            % all_full_tensor(L_idx, sample_idx, :, :, :) = target_data;

            all_masked_tensor(sample_idx, :, :, :) = input_data;
            all_full_tensor(sample_idx, :, :, :) = target_data;
            
            % 也存储到cell array (向后兼容)
            % all_masked_channels{L_idx, sample_idx} = input_data;
            % all_full_channels{L_idx, sample_idx} = target_data;

            all_masked_channels{sample_idx} = input_data;
            all_full_channels{sample_idx} = target_data;
        end
    end
    ...
    save(output_filename, ...
        ...
        'L_values', 'num_samples_per_L', ...          % SNR信息
